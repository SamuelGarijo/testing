...
class Converter {
   ...
   */
  getNumPasses() {
    if (this.cf == ImageMode.RGB565A8) return 2;
    else return 1;
  }
...
async function convertImageBlob(
  ...
  const canvas = createCanvas(isImage(img, options) ? img.width : 0, isImage(img, options) ? img.height : 0);
  const ctx = canvas.getContext("2d");
  ctx.drawImage(isImage(img, options) ? img : new Image(), 0, 0);
  const imageData = ctx.getImageData(
    0,
    0,
    isImage(img, options) ? img.width : 0,
    isImage(img, options) ? img.height : 0
  ).data;
