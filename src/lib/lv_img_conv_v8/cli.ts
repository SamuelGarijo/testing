...
const argv = yargs
  ...
  .option("binary-format", {
    ...
    string: true,
    choices: Object.keys(ImageMode)
      .filter((v) => isNaN(v as any) && v.startsWith(BINARY_FORMAT_PREFIX))
      .map((v) => v.substring(BINARY_FORMAT_PREFIX.length)) /* skip internal formats */,
  })
  .option("swap-endian", {
    alias: "s",
    type: "boolean",
...
function getFileName(imagePath) {
  ...
  if (lastDotPos < 0) lastDotPos = imagePath.length;
...
async function convertAllImages() {
  ...
  const colorFormat: ImageMode = ImageMode[argv["color-format"] as keyof typeof ImageMode];
  ...
  for (const imagePath of argv._) {
    ...
    const cFileString = await convert(imagePath, {
      cf: colorFormat,
      outputFormat: outputMode,
      binaryFormat: ImageMode[BINARY_FORMAT_PREFIX + binaryFormat],
      swapEndian: argv.s as boolean,
      outName: imageName,
      dith: argv.dither,
    });
    ...
    if (typeof cFileString == "string") fs.writeFileSync(outputPath, cFileString);
    else fs.writeFileSync(outputPath, new Uint8Array(cFileString as ArrayBuffer));
  }
}
