...
const subscribeHandler = async (req, res) => {
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      console.error(`${error.response?.status}`, `${error.response?.data.title}`, `${error.response?.data.detail}`);
    ...
    return res.status(500).json({
      error:
        "Oops! There was an error subscribing you to the newsletter. Please email us at lvgl@lvgl.io and we'll add you to the list.",
    });
  }
};
